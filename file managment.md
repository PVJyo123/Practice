...
create new user--- sudo adduser user1
run code---- sudo ./a.out
to check the change----stat file.txt
...
if both the files have the same inode number then we can say hardlink is created.
to see the hardlink---- ls -li original.txt hardlink.txt
...
truncate to change the size of the given file
to see the output---- sudo./a.out 
to see the changed size of the file stat filename
...
int main()
{
        ...
        strftime(dirname, sizeof(dirname), "%Y-%m-%d", tinfo);
...
{
        int fd;
        fd=mkdir("pictures",0777);
        if(fd==0)
                printf("created in parent class\n");
        else
...
A symbolic link is a reference to another file’s path, not its actual data.
...
int main() {
    if (truncate("file.txt", 30) == 0)
        printf("file.txt is now truncated to 30 bytes\n");
    else
...
int fd,ret;
char buf[64];
fd=open("binary.bin",O_CREAT|O_RDWR, 0660);
if(fd<0)
{
printf("faild to open the file\n");
return;
...
{
        int fd;
        int fd1=open("config.ini",O_CREAT);
        fd=access("config.ini", W_OK);
        if(fd==0)
                printf("the file is writable.\n");
        else
                printf("write acess failed\n");
}

to see ls-l config.ini
...
int main() {
    FILE *fp;
    char command[256];
    fp = fopen("instructions.txt", "r");    //open file in read mode
    if (fp == NULL)
        printf("Failed to open instructions.txt");

    printf("Executing commands from instructions.txt...\n");
    while (fgets(command, sizeof(command), fp))    // Read and execute each line as a shell command
    {
        command[strcspn(command, "\n")] = 0;   //remove traling newline
        if (command[0] == '\0')
...
need to type echo at the starting(?) 
